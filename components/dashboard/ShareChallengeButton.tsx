...
/** Compartir el reto de lectura: enlace /reto/[goalId], imagen, Instagram, Web Share. */
export function ShareChallengeButton({ goalId }: { goalId: string }) {
    const [open, setOpen] = React.useState(false);
    const [copied, setCopied] = React.useState(false);

    const url = typeof window !== "undefined" ? `${window.location.origin}/reto/${goalId}` : `/reto/${goalId}`;
    const imageUrl = `/reto/${goalId}/image`;
    ...
    const webShare = async () => {
        try {
            await navigator.share({ title: "Mi reto de lectura en Wordelia", url });
        } catch {
            /* cancelado */
        }
    };
